...
export default class AdminBloodBankMakerScreen extends React.Component{
    state = {
    ...
    save = async() => {
        try{
            if(this.state.bank == "" || this.state.phone == ""){ Toast.show("Fill form correctly!"); return; }
            this.setState({ refreshing:true, spinner:true, spinnerTitle: "saving bed info.."});
            const url = Utils.serverUrl() + "add-blood-bank?bank_name="+ this.state.bank + "&phone=" + this.state.phone+ "&mobile=" + this.state.mobile + "&email=" + this.state.email+ "&website=" + this.state.website;
            const response = await fetch(url);
            const json = await response.json();
            if(json.status == 200){ this.setState({bank:"", phone:"", mobile:"", email:"", website:""}); Alert.alert("Info", json.content); }
                    ...
                    :
                        <ScrollView style={{ flex: 1, backgroundColor:"#fff" }}>
                            <Card style={{ alignSelf:"stretch", marginBottom:20 }}>
                                <Text style={{ marginLeft:10, fontSize:18, color:"#3578e5", marginTop:8 }}>Bank Name *</Text>
                                <Input placeholder='Enter service name' onChangeText={(v)=>{ this.setState({bank:v}) }} value={this.state.bank}/>
